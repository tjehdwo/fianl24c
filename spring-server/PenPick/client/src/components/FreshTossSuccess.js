...
export default function FreshTossSuccess() {
  const [isConfirmed, setIsConfirmed] = useState(false);
  const [searchParams] = useSearchParams();
  const paymentKey = searchParams.get("paymentKey");

  const sessionTotalPrice = window.sessionStorage.getItem("totalprice");

  const [totalprice, setTotalprice] = useState(sessionTotalPrice || "");
  console.log("총가격은", totalprice);

  const sessionResnum = window.sessionStorage.getItem("id");

  const [resnum, setResnum] = useState(sessionResnum || "");
  console.log("번호는", resnum);
        ...
        >
          ...
          <div className="response-section w-100">
            <div className="flex justify-between">
              ...
              <span id="totalprice" className="response-text">
                {totalprice}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="response-label">주문번호</span>
              <span id="resnum" className="response-text">
                {resnum}
              </span>
            </div>
            <div className="flex justify-between">
